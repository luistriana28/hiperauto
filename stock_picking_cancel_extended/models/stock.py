...
class Picking(models.Model):
    ...
    def action_cancel(self):
        for line in self:
            if line.state == 'done':
                line.mapped(
                    'move_lines')._action_cancel_done()
        ...
        for move in self:
            if self.user_has_groups('stock_picking_cancel_extended.group_picking_cancel'):
                if move.state == 'cancel':
                    # We may have cancelled move in an open picking
                    # in a "propagate_cancel" scenario.
                    continue
                if move.state == 'done':
                    if move.scrapped:
                ...
                if move.state == 'cancel':
                    # We may have cancelled move in an
                    # open picking in a "propagate_cancel" scenario.
                    continue
                if move.state == 'done':
                    if move.scrapped:
                        # We may have done move in an open picking in a scrap scenario.
